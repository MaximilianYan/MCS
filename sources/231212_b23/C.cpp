...
int main() {
    ...
    vector<vector<double>> field(n, vector<double>(n, 0));
    ...
    while (true) {
        ...
        for (Edge edge : edges[next]) {
            field[start][edge.getV2()] = max(field[start][edge.getV2()], field[start][next] * edge.getW());
        }
    }

    cout << setprecision(6) << 1.0 - field[start][finish] << endl;
